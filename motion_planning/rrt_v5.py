...
class MotionPlanning(Drone):
    ...
    def Goal_Path(self, grid, rrt, rrt_path, x_near):       
        print ("Goal Found.")
        edg_rrt = nx.edges
        ...
        if found_goal == 1: 
            ...
            x_start = [0,0,0,0,0]
            edg_rrt = 500 #RRT.num_vertices
            rrt_path = RRT.edges
            RRT.
            #for (g1, g2) in edg_rrt:
            #for (g1, g2) in range(edg_rrt,0,-1):
            
            for (g1, g2) in range(nx.number_of_edges,0,-1):
                print ("edges")
                if  np.linalg.norm(RRT.x_near([g1], [g2]) < RRT.edges([g1], [g2])):
                    
                    print ("norm", np.linalg.norm(RRT.x_near([g1], [g2]) < RRT.edges([g1], [g2]))
                ...
                """ for (g1, g2) in range(edg_rrt,0,-1):
                    ...
                    if  np.linalg.norm(RRT.x_near[g1, g2])  < RRT.edges[g1, g2]:
                        ...
                        print("rrt_path edge norm", np.linalg.norm(RRT.x_near[g1, g2]) ) """
